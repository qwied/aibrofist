...
console.log('11) быстрая капля не прошивает тонкий блок насквозь');
{
  ...
  for (let k = 0; k < 500 && (GAME.pourParts.length || pools().some(o => o._wvy)); k++) ticks(16);
  /* осевшая вода — частицы жидкости, лежащие НА плите. Внизу под самóй
     плитой протечек быть не может; частицы за краями (x < 400 / x > 520)
     — это законные струйки, стёкшие с кромки и падающие в пустоту */
  const onTop = GAME.liq.filter(p => p.y < 501 && p.x > 398 && p.x < 522);
  const below = GAME.liq.filter(p => p.y > 506 && p.x > 402 && p.x < 518);
  check('капли осели НА плите', onTop.length >= 10, 'сверху: ' + onTop.length);
  check('сквозь плиту ничего не протекло', below.length === 0,
        below.map(p => p.x + '|' + p.y.toFixed(0)).join(' '));
